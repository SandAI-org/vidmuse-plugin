...
For maintainers: `npm run validate:skill` checks skills, and `npm run package:codex-plugin` builds `dist/vidmuse-plugin.zip` from git HEAD. See [SKILLS.md](./SKILLS.md) to refresh vendored domain skills. Current plugin version: **0.3.10** (see `.codex-plugin/plugin.json`).
