...
| `vidmuse-create` | Films **without** source footage: knowledge explainers / promos / **Vox paper-collage** B-roll & explainers. Non-Vox uses agency pre-production (truth → brief → 3 treatments → real storyboard → approved animatic), then the hard `vidmuse` TTS → ATA voice spine and path-routed anti-PPT craft (`path-routing` SSOT: shot_sequence + camera/continuity + hero throughline + audio_delivery + UI proof path + machine-gated semantic alignment/global-wash detection; **Vox frozen** / **recut untouched**); HyperFrames/GSAP + Timeline delivery |
