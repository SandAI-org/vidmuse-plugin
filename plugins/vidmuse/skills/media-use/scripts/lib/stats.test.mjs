...
test("buildStats aggregates resolves, misses, providers, sources, and reuse", () => {
  ...
  try {
    seedManifest(projectDir, [
      {
        ...
        source: "search",
        ts: isoDaysAgo(0),
        provenance: { provider: "vidmuse.model", prompt: "upbeat", via: "url" },
      },
      {
        id: "image_001",
        type: "image",
    ...
    seedManifest(home, [
      {
        ...
        provenance: { provider: "vidmuse.model", reused_by: "agent" },
      },
    ]);
    writeFileSync(
    ...
    assert.equal(stats.by_provider["vidmuse.model"], 1);
