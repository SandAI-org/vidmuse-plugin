#!/usr/bin/env node
/**
 * Recipes CLI — the heavyweight tier of VidMuse project memory.
 *
 *   node recipe.mjs freeze --project . --name <n> [--workflow <w>] [--blocks a,b,c]
 ...
 *
 *   node recipe.mjs list --project . [--workflow <w>] [--json]
 *     Two-tier merged listing (project wins), newest approval first.
 *
 *   node recipe.mjs use --project . --name <n> [--json]
 ...
 *
 * The owning VidMuse product workflow decides when to offer or adopt a recipe.
 */
...
try {
  if (verb === "freeze") {
    ...
    const list = listRecipes({ projectDir, workflow: args.workflow });
    ...
    const used = useRecipe({ projectDir, name: args.name });
    ...
    fail(
      "usage: recipe.mjs <freeze|list|use> --project . [--name <n>] [--workflow <w>] [--blocks a,b] [--json]",
    );
  }
} catch (err) {
