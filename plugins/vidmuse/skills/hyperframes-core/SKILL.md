...
| `references/brief-contract.md`       | the brief's ground rules — mode derivation (collaborative / autonomous), shared field registry, question invariants (VidMuse packaging briefs are owned by `/vidmuse-recut`; upstream intent live in frozen `/hyperframes` references only) |
