...
If a beat's text animation is simple enough to describe in prose ("headline fades up word-by-word, 80ms stagger"), implement it inline using the project `FRAME.md`, `vidmuse-design/references/motion-temperament.md`, and this skill's `techniques.md`, entry #4 "Per-Word Kinetic Typography". The upstream catalog is most valuable when:
