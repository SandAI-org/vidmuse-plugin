...
解释动画和声音共同构成的影片。默认使用有完整场景、证据、声音和转场
规划的 Director；这不等于持续铺满特效，安静和源画面主导仍然是正式
导演决策。用户明确要求克制的叠加式处理时再使用 Packaging。
...
新项目会先把三种基于真实内容的方向做到同一个
`frame-showcase.html` 里，推荐一个并让用户三选一；选定后再把获胜方向
写入唯一的 `FRAME.md`，并在同一个 HTML 中展开完整设计系统。不会为三
个候选各写一套 FRAME、JSON 或 Showcase。
