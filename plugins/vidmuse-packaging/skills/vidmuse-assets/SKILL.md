...
- generic stock or decorative marketing images that do not prove the beat;
...
- Let `vidmuse-media` download, generate, transform, or verify an approved media artifact when implemented.
- Let `hyperframes-cli` execute official website capture and let `vidmuse-timeline` assemble or render the film.
- Treat HyperFrames Registry items as reusable visual units, not as the raw asset library.
- Do not download, generate, transform, cache, animate, compose, or render files here.
