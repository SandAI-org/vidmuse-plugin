# Official Talking Head Recut Protocol

Pinned from HyperFrames commit `343c02518889f46ee3962256b19ac4189264907d`. VidMuse preserves this protocol and its bundled resources. Provider substitutions are limited to transcription/media execution and review UI; artifact shapes, design references, composition behavior, and rendering rules remain unchanged.

## Contents

- Workflow and official artifacts
- Storyboard and render strategy
- Card HTML and animation contract
- Composition assembly and QA
- Render and delivery

The official Step 6/7 creative workflow (card-count formula, visual-direction
questionnaire, style × layout × frame library guide, layout recipes) lives in
[legacy-creative-heuristics.md](./legacy-creative-heuristics.md) and is read
...
> `vidmuse` owns routing. This protocol packages an **existing talking-head clip** with **designed graphic cards** (titles, lower-thirds, data callouts, quotes, side panels, PiP) — not plain captions. **The clip plays untouched.**
...
The VidMuse owner confirms the input clip. Card count follows from beat selection in Step 6; aspect, per-moment layout, and visual direction are settled at Step 7 from the probed footage and transcript (see `SKILL.md` for the creative rules that govern them).

Inspectable intermediate files in the work directory:

- `metadata.json` — duration / width / height / fps
- `audio.mp3` — extracted audio
- `transcript.json` — a flat **word array** `[{ text, start, end }, …]` (no `segments`, no `words` wrapper)
...
Use `vidmuse-media` for metadata, audio extraction, and transcription while preserving the official output shapes. Use HyperFrames only for composition validation, snapshots, and local rendering.
...
Transcription is provided by `vidmuse-media` in Step 4.
...
Use `vidmuse-media` to write a word-level `transcript.json` into the work directory. Preserve the official flat-array shape: each item has `text`, `start`, and `end`; there is no `segments` or `words` wrapper. Read it for the timings that drive card timing in Step 6. Group words into sentences at punctuation or pauses when segment-level chunks are needed.

**Clamp to media duration.** A transcript provider can return the final word's `end` a hair past the actual clip length — clamp every card `endSec` and `composition.durationSeconds` to the
...
Output ratio, layout, style, and card count are decided by the VidMuse
Recut owner from the transcript and source audit (see `SKILL.md`); the
official user questionnaire and its layout/style/card-count presets have
moved to [legacy-creative-heuristics.md](./legacy-creative-heuristics.md)
and apply only when the user activates the shipped design gallery.

**Resolve the output canvas** from the chosen ratio — these are the exact
`storyboard.composition.width / height` values to write:

| ratio  | composition.width × height | storyboard.layout field                                       |
| ------ | -------------------------- | ------------------------------------------------------------- |
| `16:9` | **1920 × 1080**            | `"landscape"`                                                 |
| `9:16` | **1080 × 1920**            | `"portrait"`                                                  |
| `4:5`  | **1080 × 1350**            | `"portrait"` (schema treats 4:5 as portrait — height > width) |

For 4:5 (1080×1350) bounds, derive from portrait values by keeping
horizontal values and scaling vertical values by `1350/1920 ≈ 0.703`.

#### Render Strategy Inputs

With ratio, canvas, and the creative direction settled, the remaining
per-card decisions are:
...
- **`card.zone` per card**: choose per moment from the Step 6 zone table
  (fullscreen for hero / quote, whiteboard-area for dense data,
  lower-third for annotation over visible video, side-panel for
  sustained parallel attention, video-overlay for full-bleed
  annotation). Only when the user has activated a gallery layout recipe,
  derive it from that layout instead (split → side-panel, stack →
  lower-third, pip → fullscreen, overlay → video-overlay).
...
The composition exposes its palette as `--accent-N` / `--bg` / `--text`
CSS variables in the `<style>` block. In a bespoke direction, fill those
variables with the `FRAME.md` tokens derived from the source audit. The
official `themeId` palettes below are the gallery defaults:
...
Bundled fonts (woff2 in `<SKILL_DIR>/assets/fonts/`, staged to work dir in Step 9): `Caveat` (handwriting),
...
A bespoke `FRAME.md` may instead stage families from the `vidmuse-design`
font pack; declare whichever set the project actually uses with the same
local `@font-face` pattern (never a remote URL).

The official style × layout × frame reference library
(`references/styles|layouts|frames/`, indexed by
[DESIGN_INDEX.md](./DESIGN_INDEX.md)) is an optional user-facing gallery.
Its usage guide has moved to
[legacy-creative-heuristics.md](./legacy-creative-heuristics.md); read
either only when the user explicitly activates the shipped gallery.
...
Exact per-layout `zone` + `#video-wrap` tween recipes live in
`references/layouts/*.html` and
[legacy-creative-heuristics.md](./legacy-creative-heuristics.md) (gallery
only). For a bespoke composition, resolve `card.zone` per the Step 6 table
and author the `#video-wrap` tween directly (see Step 9).
...
- Agent-side visual-direction decisions (the bespoke `FRAME.md` choices, or a gallery pick like "overlay × geom × clean") do NOT belong in `storyboard.json` — keep them in `FRAME.md` or working memory and use them when authoring card HTML + GSAP tweens.
...
```css
/* Pattern A: transparent root, page body provides the --bg backdrop */
html,
body {
  background: var(--bg);
}
...
Landscape-baseline sizes read smaller on a phone held close. When
`storyboard.layout = "portrait"` (1080×1920, the dominant case for
social / mobile), **scale every visual size up** from its 1920×1080
landscape baseline — this applies to bespoke cards and to the gallery's
`references/styles/*.html` (all sized for landscape preview) alike.
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      :root {
        /* Bespoke runs fill these from FRAME.md tokens; gallery runs pick a
           themeId palette from the Step 7 table (example shown: classic) */
        --bg: #fff9e3;
        --text: #1e1e1e;
        --accent-0: #1971c2;
        --accent-1: #e03131;
...
In a bespoke composition, derive each `#video-wrap` target rect from the
interval's protected regions and stable whitespace (per `SKILL.md` and
`FRAME.md`) rather than from a named layout. The gallery's decorative
frames (`clean` / `hairline` / `polaroid`, in
[`references/frames/`](frames/)) sit as a **sibling** of `#video-wrap`
and follow it through layout transitions; they apply only when the user
activates the gallery. Quick rule there: `overlay` layout suppresses
decorative frames (the full-bleed video clashes with chrome); PiP
layouts already have their own pill treatment (border-radius + white
ring + shadow), so add a decorative frame only on top of `split` /
`stack`.

**GSAP target lookup table** for `#video-wrap` per gallery layout
(landscape 1920×1080 — for portrait & 4:5 see `references/layouts/*.html`
which list all three ratios). In a bespoke composition, treat these rows
as mechanism examples of tween targets, not as the set of available
positions:
...
**Review.** Inspect representative HyperFrames snapshots, then hand `public/index.html` and the official artifacts to `vidmuse-timeline`. Never start HyperFrames Studio, `hyperframes preview`, or its Timeline UI.
