...
Words in a phrase visually activate (glow blur + scale) when "spoken", following an attack-sustain-release envelope over per-word `{ start, end }` timestamps. Supply word timings through `vidmuse-media` or author them by hand; do not run HyperFrames transcription or model commands. The envelope never falls to zero after a word — it decays to a rest level, leaving a breadcrumb of recent emphasis.
...
`3d-text-depth-layers` (depth on the active word at peak) · `sine-wave-loop` (idle breathe between emphasis moments) · `context-sensitive-cursor` (typewriter matching the ASR cadence) · `vidmuse-media` for transcript timings and caption media.
