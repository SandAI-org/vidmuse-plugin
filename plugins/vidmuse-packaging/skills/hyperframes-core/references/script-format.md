...
This file defines the SCRIPT.md **shape** only. Synthesizing spoken lines into audio is owned by `vidmuse-media`.

Free-form markdown — there is no strict parser. The VidMuse media step extracts the indented spoken lines.
...
Pass each spoken line to `vidmuse-media`. Do not run HyperFrames TTS, transcription, or model commands. Real per-word timing replaces the `**Time:**` guides.
