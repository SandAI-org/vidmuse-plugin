...
Rendering remains user-gated in both modes. After checks pass and representative snapshots are available, ask “render now, or what changes?” Render only after the answer.

### Checkpoint feedback

Checkpoint feedback arrives in chat or alongside the shared snapshot/contact sheet. Apply only the named frame changes and re-present the affected frames.

Autonomous is not silent: replace absorbed questions with visible decisions and short reasons. Every autonomous visual or video delivery names the rendered artifact as applicable, reports the actual duration for a time-based deliverable, and includes a contact sheet or snapshot sheet plus relevant frame identifiers when available. For multi-scene work, use scene midpoints; for a single-scene piece, use one or more proof times.
